...
Enter the development shell explicitly with `nix develop`, then run:
...
To run a single command without entering an interactive shell, use
`nix develop -c <command>`, for example `nix develop -c pnpm lint`.

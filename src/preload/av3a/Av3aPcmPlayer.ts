 ...
 * matching the official client: a per-channel decode matrix feeding a
 * lookahead peak limiter per side (instead of a hard clamp).
 *
 * Channel order follows the AV3A bed layout: 0=L, 1=R, 2=C, 3=LFE, 4=Lss,
 * 5=Rss, 6=Lrs, 7=Rrs, 8=Ltf, 9=Rtf, 10=Ltb, 11=Rtb. Only the first 12
 * channels are mapped.
 */

/** Trim applied to every channel (official client). */
const TRIM = 0.85;
const G = TRIM / Math.SQRT2;

/** Stereo decode matrix: [leftGain, rightGain] per bed channel. */
const STEREO_GAINS: [number, number][] = [
  [TRIM, 0], // 0 L
  [0, TRIM], // 1 R
  [TRIM, TRIM], // 2 C
  [TRIM, TRIM], // 3 LFE
...
/**
 * Lookahead peak limiter for one stereo side. Samples are processed in PCM16
 * units (~±32768). The output is delayed by one ring length so the gain —
 * derived from the incoming (undelayed) envelope — is already applied by the
 * time the loud sample reaches the output.
 */
class Limiter {
  ...
  constructor(private readonly len = 100) {
  ...
  processSample(sample: number): number {
    ...
    const read = (this.write + 1) % this.len;
    this.write = read;

    // Envelope decay (~0.01%/sample): track the peak, fall back slowly.
    this.envelope = Math.max(this.envelope * 0.9998999834, Math.abs(sample));
    // Target gain: reduce once the envelope exceeds the ceiling (~0.9999 LSB).
    const ceiling = 32764.72266;
    const target = this.envelope > ceiling ? ceiling / this.envelope : 1.0;
    this.gain = (1 - 0.001) * target + 0.001 * this.gain;
    return this.gain * this.ring[read];
  }
}

const leftLimiter = new Limiter();
const rightLimiter = new Limiter();

/** Reset the limiter state (a fresh track / seek must not inherit gain). */
...
function pcm16ToStereoFloat32(
  ...
  for (let i = 0; i < frames; i++) {
    ...
    let r = 0;
    for (let c = 0; c < ch; c++) {
      const gain = STEREO_GAINS[c];
      if (!gain) continue;
      const sample = src[base + c] * scale;
      l += sample * gain[0];
      r += sample * gain[1];
    }

    const processedL = leftLimiter.processSample(l * 32768) / 32768;
    const processedR = rightLimiter.processSample(r * 32768) / 32768;

    out[i * 2] = processedL;
    out[i * 2 + 1] = processedR;
  }
  return out;
}
